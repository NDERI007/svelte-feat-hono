<script lang="ts">
  ...
  import { searchPlaces, type PlaceResult } from '$lib/apis/places';
  ...
  async function handleInput() {
    ...
    debounceTimer = setTimeout(async () => {
      ...
      try {
        ...
        results = await searchPlaces(query, sessionToken, 10);
        isOpen = true;
        selectedIndex = -1;
      } catch (error) {
...
<div class="relative w-full">
  <!-- Search Input -->
  <div class="flex items-center gap-2 rounded-full bg-white px-4 py-3 shadow-md {disabled ? 'opacity-50 cursor-not-allowed' : ''}">
    ...
    >
      {#if results.length > 0}
        {#each results as place, index (place.place_id || place.id || index)}
          <li
            class="cursor-pointer px-4 py-2 text-left transition-colors {selectedIndex === index ? 'bg-gray-100' : 'hover:bg-gray-50'}"
            onmousedown={(e) => {
              e.preventDefault();
              selectPlace(place);
            }}
          ...
          >
            <div class="flex items-start justify-between gap-2">
              ...
              <span class="px-2 py-0.5 text-xs font-medium rounded {place.source === 'db' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}">
